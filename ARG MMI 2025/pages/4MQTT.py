...
def parse_sensor_data(text):
    try:
        ...
        data["temp"] = float(re.search(r"Temp\s*=\s*([\d,\.]+)", text).group(1).replace(',', '.'))
...
st.set_page_config(page_title="MQTT + Google Sheets", layout="wide")
st.title("📡 MONITORING REALTIME AWS 2025")
